...
* **El Panel de** estadísticas (que se puede ver en La vista de juegos en "Estadísticas") le mostrará la cantidad de lotes o lotes guardados, las llamadas **SetPass y la velocidad de fotogramas**.
    ...
    * Lotes guardados (solo visibles cuando se reproduce la escena): la cantidad de llamadas a draw que se han reducido mediante el procesamiento por lotes estático o la creación de **instancias de GPU.**
    * Llamadas SetPass: el número de materiales visibles diferentes en una escena
    * Velocidad de fotogramas: la cantidad de fotogramas por segundo en la vista De juego (te ofrece una idea aproximada de lo que sucede; las escenas siempre se deben probar en la aplicación, en el casco, con el panel Velocidad de fotogramas de Oculus, ya que la lectura de fps siempre será diferente de lo que hay en el editor).

* **Depurador de fotogramas** (se encuentra en Ventana/Análisis/Depurador de fotogramas).  El Panel de estadísticas de la ciudad que, cuando esté habilitado, le permitirá ver lo que dibuja la GPU para crear la imagen final, mostrando una lista de llamadas a draw del primero al último.  Le dará motivos para que una llamada a draw no se procesara por lotes con una llamada a draw anterior (es decir, "Este objeto usa un material diferente" o "Este objeto usa un mapa claro diferente"), y es una excelente manera de desarrollar un conocimiento de lo que sucede en la escena y cómo y por qué determinadas opciones visuales pueden ser costosas desde el punto de vista computacional.
...
* Por lo general, una resolución de mapa de luz de aproximadamente **25 elementos** de textura por unidad debe ser suficiente (establezca la resolución en la configuración de iluminación y escena).  Si tiene espacio adicional en el mapa de luz, puede aumentar este valor.
...
* **Gráfico de escalado de mapa de luz** (regla general) 
    ...
    * Propiedades (especialmente propiedades más pequeñas que una persona): **de 2** a 3 (para evitar **artefactos** de mapa de luz y juntas en los objetos) 
...
Nota: Si usa esta configuración, puede establecer Lightmapper en "GPU Lightmapper" y establecer lightmap Size en "2048" para realizar "bakes" en versión preliminar mucho más rápidos y, a continuación, realizar una copia de seguridad en CPU y 4k para la última".

## <a name="texture-compressionfile-size"></a>**Compresión de textura/tamaño de archivo**

* Para nuestra compilación de Android, intentamos mantener el tamaño de la escena del paquete de Unity en torno a 10-20 MB en total.  Para ello, compartimos materiales genéricos entre muchos objetos, usamos el color de vértice para tejer los objetos y también establecemos invalidaciones manuales para Android para que las texturas usen la compresión de bloque **ASTC 6x6,** que será menor que la compresión predeterminada.

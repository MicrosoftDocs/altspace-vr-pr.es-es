...
| Interfaz de usuario (UI)/Menu | Cuando se encuentra en AltspaceVR en el mundo, dentro del entorno envolvente, hay menús a la izquierda y a la derecha de la pantalla. El círculo o el menú principal con el logotipo altspaceVR abre la interfaz de usuario (UI) principal o el menú para acceder a diferentes pantallas para explorar AltspaceVR y personalizar su experiencia. Los elementos opcionales de la interfaz de usuario se encuentran en el tamaño correcto de la pantalla y suelen incluir World Editor y Host Tools. Estos se abren e interactúan haciendo clic en ellos con el cursor. |
| SDK/MRE | Estos son los términos de creación del mundo asociados con el Kit de desarrollo de software [y Mixed Reality Extensions](../world-building/using-mixed-reality-extensions.md) que se usan para agregar características y funcionalidades a la experiencia de creación mundial. Normalmente son para usuarios más avanzados. |
...
| Término | Definición |
|---|---|
| World copied (Mundo copiado) | El uso de un espacio del mundo para un evento repetido permite una personalización persistente. Los signos, las imágenes, los puntos de generación y otras personalizaciones se mantienen de evento a evento en lugar de personalizar el espacio del evento cada vez. |
| Personalización del mundo de los eventos | Cuando se crea un evento, la plantilla o el mundo importado se copia y se bloquea en ese evento. Puede realizar cambios en el mundo de los eventos y no afectar al mundo original y al revés. Considere la posibilidad de agregar decoración de vacaciones, imágenes u otros elementos del editor del mundo de uso único para que el espacio del evento sea más divertido y adecuado para el evento. |
...
3. Si el espacio de eventos no ha cambiado, vaya a Configuración > Restablecer > **restablecer** espacio para restablecer el espacio de eventos. Restablecerá mucho el espacio como un creador del mundo.
...
En la [página web Eventos >](https://account.altvr.com/events/my) Mis eventos en el  sitio web de AltspaceVR, seleccione Programar un evento o vaya directamente a la página web Create Event in AltspaceVR (Crear evento [en AltspaceVR).](https://account.altvr.com/events/new)
...
5. Establezca el evento en **Privado** o **Público.**
    * Visibilidad: los eventos públicos están visibles en la pestaña Todos del calendario de eventos del sitio web altspaceVR [y](https://account.altvr.com/events/all) están abiertos al público.
...
1. **Promover:** Cada evento requiere dos imágenes de personal de marca (estas imágenes aparecen en el sitio web altspaceVR y no aparecen en el evento dentro de AltspaceVR):
    ...
    * Otra opción (más rápida) es copiar la imagen de icono en la imagen de banner de **fondo,** que también usará la imagen de icono como imagen de banner. Pruébalo :). Si no tiene buen aspecto, puede crear una nueva imagen de Banner.
...
3. **Avanzado:** A continuación se muestra una explicación rápida de las características avanzadas de los eventos (algunas de estas características no se mostrarán hasta después de crear el evento y edite el evento):
    * **Administradores:** Los administradores son usuarios de AltspaceVR en los que confía para ayudarle a administrar el evento. Pueden ser los cohosts o los hosts de copia de seguridad. Al agregar su nombre de usuario a la lista De administradores, pueden:
        ...
        * Elimine el evento.
    * **Grupo:** Elija entre los grupos [privados](group-features.md) mediante el menú desplegable. Solo se mostrará si la cuenta se ha creado o se ha agregado a un grupo.
...
3. **Identificador de vídeo de YouTube:** Visible en la página web del evento, agrega finalizadores de eventos de YouTube o vídeos a la página web del evento, no en el mundo. Solo necesitará la parte del identificador de vídeo de la dirección URL: dQw4w9WgXcQ.
4. **Identificador de Twitter:** Esto agrega la secuencia de Twitter a la página web del evento. Si la cuenta de Twitter es personal y no está relacionada con el evento o la asociación, puede que esté compartiendo en exceso. Solo necesitará el identificador: @elonmusk
...
5. Seleccione **CREATE PHOTO**.
6. Repita esta opción para todas las imágenes.

En AltspaceVR y en el mundo de los eventos, colócate cerca de donde quieras colocar la imagen. (Si va a crear esto para un evento **de** uso repetido, asegúrese de que está en su mundo y no en el espacio de eventos).

1. En la interfaz de usuario, en la esquina inferior derecha, seleccione **Editor mundial/Panel del editor.**
...
5. Con el cursor del dispositivo, tome la imagen y colócala donde quiera que esté.
    * Puede cambiar su tamaño con los controladores (normalmente deslizando el dedo horizontalmente por el panel táctil en VR o con el mouse en modo 2D).
...
6. Repita este paso para cada ubicación de imagen.
7. Cuando las imágenes estén establecidas, seleccione Bloquear todo si no las bloqueó individualmente, desactive El modo de edición y cierre el editor. 
8. Use un máximo de cinco Fotos por evento.

## <a name="adding-3d-objects-using-world-editor-and-kits"></a>Adición de objetos 3D mediante el editor world y kits

AltspaceVR permite importar objetos 3D a mundos con el Editor del mundo. Hay muchos kits de editores del mundo disponibles actualmente para su uso y mucho más en el camino. Hay naves espaciales, cohetes, animales animados, árboles, plantas y muchos objetos diferentes. O bien, puede agregar sus propios kits a la colección pública o para su propio uso personal, o bien considerar la posibilidad de usar una de las aplicaciones de [MRE altspaceVR,](../world-building/using-mixed-reality-extensions.md)que incluyen objetos interactuables en su mundo, como cascos, accesorios, alerones, ropa, cascos u otros objetos interactuables. Espere que la lista de objetos se expanda pronto.

Las instrucciones que se indican aquí se centran en el uso de elementos que ya están en los kits. Para obtener más información sobre cómo agregar e importar objetos 3D en AltspaceVR, consulte ["How to import glTF models into my Worlds?"](../world-building/importing-models.md) (Cómo importar modelos glTF a mis mundos). y ["Cómo cargar mis propios kits?".](../world-building/uploading-custom-kits.md)

Para agregar un objeto 3D existente de World Building Kits a su mundo de eventos, con el editor del mundo abierto y el modo de edición habilitados:

1. Explore los kits del objeto que desea importar y selecciónelo.
2. Seleccione en el objeto de kit con el cursor y posiciónelo. Use el icono DE ENGRANAJE del objeto en el Editor del mundo para usar las opciones Posición, Girar y Escalar para la entrada de número manual.
3. Cuando el objeto se coloca en la posición correcta, puede bloquearlo haciendo clic en el icono Bloquear.
4. Agregue otro objeto si lo desea. Tenga en cuenta la adición de objetos 3D, manteniendo sus selecciones en no más de elementos de cinco kits individuales diferentes, para mejorar la compatibilidad con el usuario de Oculus Quest. No use más de 200 objetos en un mundo.
5. Cuando haya terminado, bloquee todos los elementos, desactive el modo de edición y cierre el Editor del mundo. Pasear. Invite a algunos evaluadores a visitarlo, echarle un vistazo y prepararse para su gran evento.

Para obtener más información sobre world editor kits y agregar objetos 3D a AltspaceVR:
